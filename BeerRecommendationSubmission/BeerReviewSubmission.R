...
#Description: Each record is composed of a beer's name, the name of the user along with ratings provided by users.
...
beerdata <- read.csv("beer_data.csv", stringsAsFactors = F)
...
View(beerdata)
str(beerdata) #beer_beeridid & review_overall are numeric, review_profilename(name of reviewer) is factor
summary(beerdata) #review_profilename have missing values, reviews are in range 0-5
dim(beerdata) #475984 user rating & 3 features

########################################################## Cleaning Data ###############################################

#Removing missing values
nrow(beerdata[(is.na(beerdata$review_profilename) | beerdata$review_profilename==""), ])
#Total 100 reviews have empty review_profilename
#Updating beer datset removing empty review_profilename
beerdata1<-beerdata[!(beerdata$review_profilename==""), ]
...
beerdata1[!(duplicated(beerdata1[c("beer_beerid","review_profilename")]) | duplicated(beerdata1[c("beer_beerid","review_profilename")], fromLast = TRUE)), ] %>% nrow()
beerdata1 %>% distinct(beer_beerid,review_profilename,.keep_all = TRUE) %>% nrow() #474462 distict reviews
...
beerdata1<-distinct(beerdata1,beer_beerid,review_profilename,.keep_all = TRUE)


#Lets find all distinct beers & total reviews they received
beer_reviews_count <- beerdata1 %>% group_by(beer_beerid) %>% summarise(total_beer_reviews=n())
dim(beer_reviews_count) #Total 40304 distinct beers, so 8.49% of total reviews beers are unique
summary(beer_reviews_count) 
#Beers have reviews count in range of 1 to 977, beer 77317 have highest number of reviews(977)

#Lets also count number of distinct users
beerdata1 %>% group_by(review_profilename) %>% summarise(total_user_reviews=n()) %>% nrow() #22497 distinct users

#Lets now check which user have reviewed maximum beers
beerdata1 %>% group_by(review_profilename) %>% summarise(user_review_count=n()) %>% top_n(1)
...
beer_reviews_count %>% subset(total_beer_reviews==1) %>% dim() #So 18080 or 44.7% beers have only 1 review, lets group by total reviews & find count for each
...
#Deciding value of N:
beerall_rrmatrix <- as(beerdata1[,c(2,1,3)], "realRatingMatrix")
...
#distinct beers(40304) then users(22497), 1.8 times; but taking both as 50 here based on results from similarity matrix also
...
user_reviews_count<- beerdata1 %>% group_by(review_profilename) %>% summarise(total_user_reviews=n()) 
...
important_beers<-merge(beerdata1,beer_reviews_count_subset,by.x="beer_beerid",by.y="beer_beerid")
...
avg_beer_ratings_all<-beerdata1 %>% group_by(beer_beerid) %>% summarise(average_rating=mean(review_overall))
...
#So average beer ratings(Median)=3.8 & Mean=3.671, uneven distribution
...
#Also checking on original full dataset of beers
avg_user_ratings_all<-beerdata1 %>% group_by(review_profilename) %>% summarise(average_rating=mean(review_overall))
...
#So average beer ratings(Mean)=3.990s & Median=4, uneven distribution
